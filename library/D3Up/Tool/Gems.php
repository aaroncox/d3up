<?php
/**
 * undocumented class
 *
 * @package default
 * @author Aaron Cox
 **/
class D3Up_Tool_Gems
{
	public static $gems = array(
		'chipped_amethyst' => array('Chipped Amethyst' , array('plus-life', 5) , array('life-hit', 2), array('vitality', 6)),
		'chipped_emerald' => array('Chipped Emerald' , array('plus-gold-find', 5), array('critical-hit-damage', 10), array('dexterity', 6)),
		'chipped_ruby' => array('Chipped Ruby' , array('plus-experience-percent', 5), array('ruby-damage', 3) ,array('strength',6)),
		'chipped_topaz' => array('Chipped Topaz' , array('plus-magic-find', 5), array('thorns', 2), array('intelligence', 6)),
		'flawed_amethyst' => array('Flawed Amethyst' , array('plus-life', 6), array('life-hit', 3), array('vitality', 10)),
		'flawed_emerald' => array('Flawed Emerald' , array('plus-gold-find', 7), array('critical-hit-damage', 15), array('dexterity', 10)),
		'flawed_ruby' => array('Flawed Ruby' , array('plus-experience-percent', 7), array('ruby-damage', 6), array('strength', 10)),
		'flawed_topaz' => array('Flawed Topaz' , array('plus-magic-find', 7), array('thorns', 3), array('intelligence', 10)),
		'amethyst' => array('Amethyst' , array('plus-life', 7) , array('life-hit', 6), array('vitality', 14)),
		'emerald' => array('Emerald' , array('plus-gold-find', 9), array('critical-hit-damage', 20), array('dexterity', 14)),
		'ruby' => array('Ruby' , array('plus-experience-percent', 9), array('ruby-damage', 9) ,array('strength', 14)),
		'topaz' => array('Topaz' , array('plus-magic-find', 9), array('thorns', 6), array('intelligence', 14)),
		'flawless_amethyst' => array('Flawless Amethyst' , array('plus-life', 8) , array('life-hit', 10), array('vitality', 18)),
		'flawless_emerald' => array('Flawless Emerald' , array('plus-gold-find', 11), array('critical-hit-damage', 25), array('dexterity', 18)),
		'flawless_ruby' => array('Flawless Ruby' , array('plus-experience-percent', 11), array('ruby-damage', 12) ,array('strength', 18)),
		'flawless_topaz' => array('Flawless Topaz' , array('plus-magic-find', 11), array('thorns', 10), array('intelligence', 18)),
		'perfect_amethyst' => array('Perfect Amethyst' , array('plus-life', 9) , array('life-hit', 15), array('vitality', 22)),
		'perfect_emerald' => array('Perfect Emerald', array('plus-gold-find', 13), array('critical-hit-damage', 30), array('dexterity', 22)),
		'perfect_ruby' => array('Perfect Ruby' , array('plus-experience-percent', 13), array('ruby-damage', 15) ,array('strength', 22)),
		'perfect_topaz' => array('Perfect Topaz' , array('plus-magic-find', 13), array('thorns', 15), array('intelligence', 22)),
		'radiant_amethyst' => array('Radiant Amethyst' , array('plus-life', 10) , array('life-hit', 25), array('vitality', 26)),
		'radiant_emerald' => array('Radiant Emerald', array('plus-gold-find',15), array('critical-hit-damage', 35), array('dexterity', 26)),
		'radiant_ruby' => array('Radiant Ruby' , array('plus-experience-percent', 15), array('ruby-damage', 18) ,array('strength', 26)),
		'radiant_topaz' => array('Radiant Topaz' , array('plus-magic-find', 15), array('thorns', 30), array('intelligence', 26)),
		'square_amethyst' => array('Square Amethyst' , array('plus-life', 11) , array('life-hit', 35), array('vitality', 30)),
		'square_emerald' => array('Square Emerald', array('plus-gold-find', 17), array('critical-hit-damage', 40), array('dexterity', 30)),
		'square_ruby' => array('Square Ruby' , array('plus-experience-percent', 17), array('ruby-damage', 21) ,array('strength', 30)),
		'square_topaz' => array('Square Topaz' , array('plus-magic-find', 17), array('thorns', 50), array('intelligence', 30)),
		'flawless_square_amethyst' => array('Flawless Square Amethyst' , array('plus-life', 12) , array('life-hit', 65), array('vitality', 34)),
		'flawless_square_emerald' => array('Flawless Square Emerald', array('plus-gold-find', 19), array('critical-hit-damage', 45), array('dexterity', 34)),
		'flawless_square_ruby' => array('Flawless Square Ruby' , array('plus-experience-percent', 19), array('ruby-damage', 25) ,array('strength', 34)),
		'flawless_square_topaz' => array('Flawless Square Topaz' , array('plus-magic-find', 19), array('thorns', 100), array('intelligence', 34)),
		'perfect_square_amethyst' => array('Perfect Square Amethyst' , array('plus-life', 13) ,array('life-hit', 105), array('vitality', 38)),
		'perfect_square_emerald' => array('Perfect Square Emerald' , array('plus-gold-find', 21), array('critical-hit-damage', 50), array('dexterity', 38)),
		'perfect_square_ruby' => array('Perfect Square Ruby' , array('plus-experience-percent', 21), array('ruby-damage', 30) ,array('strength', 38)),
		'perfect_square_topaz' => array('Perfect Square Topaz' , array('plus-magic-find', 21), array('thorns', 200), array('intelligence', 38)),
		'radiant_square_amethyst' => array('Radiant Square Amethyst' , array('plus-life', 14) , array('life-hit', 190), array('vitality', 42)),
		'radiant_square_emerald' => array('Radiant Square Emerald' , array('plus-gold-find', 23), array('critical-hit-damage', 60), array('dexterity', 42)),
		'radiant_square_ruby' => array('Radiant Square Ruby' , array('plus-experience-percent', 23), array('ruby-damage', 40) ,array('strength', 42)),
		'radiant_square_topaz' => array('Radiant Square Topaz' , array('plus-magic-find', 23), array('thorns', 350), array('intelligence', 42)),
		'star_amethyst' => array('Star Amethyst' , array('plus-life', 15) , array('life-hit', 300), array('vitality', 46)),
		'star_emerald' => array('Star Emerald' , array('plus-gold-find', 25), array('critical-hit-damage', 70), array('dexterity', 46)),
		'star_ruby' => array('Star Ruby' , array('plus-experience-percent', 25), array('ruby-damage', 60) ,array('strength', 46)),
		'star_topaz' => array('Star Topaz' , array('plus-magic-find', 25), array('thorns', 600), array('intelligence', 46)),
		'flawless_star_amethyst' => array('Flawless Star Amethyst' , array('plus-life', 16) , array('life-hit', 400), array('vitality', 50)),
		'flawless_star_emerald' => array('Flawless Star Emerald' , array('plus-gold-find', 27), array('critical-hit-damage', 80), array('dexterity', 50)),
		'flawless_star_ruby' => array('Flawless Star Ruby' , array('plus-experience-percent', 27), array('ruby-damage', 80) ,array('strength', 50)),
		'flawless_star_topaz' => array('Flawless Star Topaz' , array('plus-magic-find', 27), array('thorns', 900), array('intelligence', 50)),
		'perfect_star_amethyst' => array('Perfect Star Amethyst' , array('plus-life', 17) , array('life-hit', 500), array('vitality', 54)),
		'perfect_star_emerald' => array('Perfect Star Emerald', array('plus-gold-find', 29), array('critical-hit-damage', 90), array('dexterity', 54)),
		'perfect_star_ruby' => array('Perfect Star Ruby' , array('plus-experience-percent', 29), array('ruby-damage', 100) ,array('strength', 54)),
		'perfect_star_topaz' => array('Perfect Star Topaz', array('plus-magic-find', 29), array('thorns', 1250), array('intelligence', 54)),
		'radiant_star_amethyst' => array('Radiant Star Amethyst' , array('plus-life', 18) , array('life-hit', 600), array('vitality', 58)),
		'radiant_star_emerald' => array('Radiant Star Emerald' , array('plus-gold-find', 31), array('critical-hit-damage', 100), array('dexterity', 58)),
		'radiant_star_ruby' => array('Radiant Star Ruby' , array('plus-experience-percent', 31), array('ruby-damage', 130) ,array('strength', 58)),
		'radiant_star_topaz' => array('Radiant Star Topaz' , array('plus-magic-find', 31), array('thorns', 1800), array('intelligence', 58)),
		'marquise_amethyst' => array('Marquise Amethyst',array('plus-life',19),array('life-hit',700),array('vitality',62)),
		'marquise_emerald' => array('Marquise Emerald',array('plus-gold-find',33),array('critical-hit-damage',110),array('dexterity',62)),
		'marquise_ruby' => array('Marquise Ruby',array('plus-experience-percent',33),array('ruby-damage', 160),array('strength',62)),
		'marquise_topaz' => array('Marquise Topaz',array('plus-magic-find',33),array('thorns',2200),array('intelligence',62)),		
		"chipped_diamond" => array("Chipped Diamond", array("skill-cdr",3.5), array("elite-damage",2), array("resist-all",6)),
		"flawed_diamond" => array("Flawed Diamond", array("skill-cdr",4), array("elite-damage",3), array("resist-all",10)),
		"diamond" => array("Topaz", array("skill-cdr",4.5), array("elite-damage",4), array("resist-all",14)),
		"flawless_diamond" => array("Flawless Diamond", array("skill-cdr",5), array("elite-damage",5), array("resist-all",18)),
		"perfect_diamond" => array("Perfect Diamond", array("skill-cdr",5.5), array("elite-damage",6), array("resist-all",22)),
		"radiant_diamond" => array("Radiant Diamond", array("skill-cdr",6), array("elite-damage",7), array("resist-all",26)),
		"square_diamond" => array("Square Diamond", array("skill-cdr",6.5), array("elite-damage",8), array("resist-all",30)),
		"flawless_square_diamond" => array("Flawless Square Diamond", array("skill-cdr",7), array("elite-damage",9), array("resist-all",34)),
		"perfect_square_diamond" => array("Perfect Square Diamond", array("skill-cdr",7.5), array("elite-damage",10), array("resist-all",38)),
		"radiant_square_diamond" => array("Radiant Square Diamond", array("skill-cdr",8), array("elite-damage",11), array("resist-all",42)),
		"star_diamond" => array("Star Diamond", array("skill-cdr",8.5), array("elite-damage",12), array("resist-all",46)),
		"flawless_star_diamond" => array("Flawless Star Diamond", array("skill-cdr",9), array("elite-damage",13), array("resist-all",50)),
		"perfect_star_diamond" => array("Perfect Star Diamond", array("skill-cdr",9.5), array("elite-damage",14), array("resist-all",54)),
		"radiant_star_diamond" => array("Radiant Star Diamond", array("skill-cdr",10), array("elite-damage",15), array("resist-all",58)),
		"marquise_diamond" => array("Marquise Diamond", array("skill-cdr",10.5), array("elite-damage",16), array("resist-all",62)),
		"imperial_diamond" => array("Imperial Diamond", array("skill-cdr",11), array("elite-damage",17), array("resist-all",66)),
		"flawless_imperial_diamond" => array("Flawless Imperial Diamond", array("skill-cdr",11.5), array("elite-damage",18), array("resist-all",70)),
		"royal_diamond" => array("Royal Diamond", array("skill-cdr",12), array("elite-damage",19), array("resist-all",74)),
		"flawless_royal_diamond" => array("Flawless Royal Diamond", array("skill-cdr",12.5), array("elite-damage",20), array("resist-all",78)),
		"imperial_amethyst" => array("Imperial Amethyst", array("plus-life",20), array("life-hit",800), array("vitality",100)),
		"flawless_imperial_amethyst" => array("Flawless Imperial Amethyst", array("plus-life",21), array("life-hit",900), array("vitality",160)),
		"royal_amethyst" => array("Royal Amethyst", array("plus-life",22), array("life-hit",1000), array("vitality",220)),
		"flawless_royal_amethyst" => array("Flawless Royal Amethyst", array("plus-life",23), array("life-hit",1100), array("vitality",280)),
		"imperial_ruby" => array("Imperial Ruby", array("plus-experience-percent",35), array("ruby-damage",190), array("strength",100)),
		"flawless_imperial_ruby" => array("Flawless Imperial Ruby", array("plus-experience-percent",37), array("ruby-damage",220), array("strength",160)),
		"royal_ruby" => array("Royal Ruby", array("plus-experience-percent",39), array("ruby-damage",250), array("strength",220)),
		"flawless_royal_ruby" => array("Flawless Royal Ruby", array("plus-experience-percent",41), array("ruby-damage",270), array("strength",280)),
		"imperial_emerald" => array("Imperial Emerald", array("plus-gold",35), array("critical-hit-damage",115), array("dexterity",100)),
		"flawless_imperial_emerald" => array("Flawless Imperial Emerald", array("plus-gold",37), array("critical-hit-damage",120), array("dexterity",160)),
		"royal_emerald" => array("Royal Emerald", array("plus-gold",39), array("critical-hit-damage",125), array("dexterity",220)),
		"flawless_royal_emerald" => array("Flawless Royal Emerald", array("plus-gold",41), array("critical-hit-damage",130), array("dexterity",280)),
		"imperial_topaz" => array("Imperial Topaz", array("plus-magic-find",35), array("thorns",3100), array("intelligence",100)),
		"flawless_imperial_topaz" => array("Flawless Imperial Topaz", array("plus-magic-find",37), array("thorns",3725), array("intelligence",160)),
		"royal_topaz" => array("Royal Topaz", array("plus-magic-find",39), array("thorns",4350), array("intelligence",220)),
		"flawless_royal_topaz" => array("Flawless Royal Topaz", array("plus-magic-find",41), array("thorns",4975), array("intelligence",280))
		
	);
} // END class D3Up_Tool_Gems